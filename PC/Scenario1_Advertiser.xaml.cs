...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario1_Advertiser : Page
    {
        ...
        private void btnStartAdvertisement_Click(object sender, RoutedEventArgs e)
        {
            ...
            rootPage.NotifyUser("publisher try to establish...", NotifyType.StatusMessage);
            
            _publisher = new WiFiDirectAdvertisementPublisher();
            rootPage.NotifyUser("publisher established...", NotifyType.StatusMessage);
        ...
        private async void ChooseFile(object sender, RoutedEventArgs e)
        {
            ...
            CoreApplication.Properties.Add("selectfile", file);
            if (file != null)
            {
                this.txtChooseFile.Text = file.Name;
            }
            else
            {
                this.txtChooseFile.Text = "The operation has been cancelled";
            }
        }
        ...
        {
            object value;
            if (!CoreApplication.Properties.TryGetValue("selectfile", out value))
            {
                rootPage.NotifyUser("Please select the path before doing this one.", NotifyType.ErrorMessage);
                return;
            }
            StorageFile fileobj = ((StorageFile)value);

            var connectedDevice = (ConnectedDevice)lvConnectedDevices.SelectedItem;
            rootPage.NotifyUser(fileobj.Name, NotifyType.ErrorMessage);
            await connectedDevice.SocketRW.SendFileAsync(fileobj);
        }
        ...
        private async void ChoosePath(object sender, RoutedEventArgs e)
        {
            ...
            StorageFolder file = await openFile.PickSingleFolderAsync();
            CoreApplication.Properties.Add("selectpath", file);
            if (file != null)
            {
                this.txtChoosePath.Text = file.Name;
            }
            else
            {
                this.txtChoosePath.Text = "The operation has been cancelled";
            }
        }
        ...
        private async void SaveFile(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                //file handler
                object value;
                if (!CoreApplication.Properties.TryGetValue("selectpath", out value))
                {
                    rootPage.NotifyUser("Please select the path before doing this one.", NotifyType.ErrorMessage);
                    return;
                }
                StorageFolder pathobj = ((StorageFolder)value);
                string path = pathobj.Name;
                //save the file handler to the path
                rootPage.NotifyUser(
                    "Save file successfully in " + path,
                    NotifyType.StatusMessage);
            }
            catch (Exception exception)
            {
                rootPage.NotifyUser(
                    "Save file failed " + exception.Message,
